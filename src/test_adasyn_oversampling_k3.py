...
for data in keel_data:
    ...
    csv_export_file_name = "../results/adasyn_oversampling_k3/data-naive_bayes.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/adasyn_oversampling_k3/confusion_matrix/%s-%s.%s" % (
        data.file_name, "naive_bayes", "png")
    class_distribution_plot_file_name = "../results/adasyn_oversampling_k3/class_distribution/%s-%s.%s" % (
        data.file_name, "naive_bayes", "png")

    # Split dataset and perform classification
    testable = data.as_testable()
    sampled_testable = common_preprocessing.adasyn_over_sampling(testable, ADASYN_K_NEIGHBOURS)
...
for data in keel_data:
    # initialize file names and params
    csv_export_file_name = "../results/adasyn_oversampling_k3/data-random_forest.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/adasyn_oversampling_k3/confusion_matrix/%s-%s.%s" % (
...
for data in keel_data:
    # initialize file names and params
    csv_export_file_name = "../results/adasyn_oversampling_k3/data-decision_tree.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/adasyn_oversampling_k3/confusion_matrix/%s-%s.%s" % (
