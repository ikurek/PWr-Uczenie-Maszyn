...
for data in keel_data:
    ...
    csv_export_file_name = "../results/random_undersampling/data-naive_bayes.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/random_undersampling/confusion_matrix/%s-%s.%s" % (
        data.file_name, "naive_bayes", "png")
    class_distribution_plot_file_name = "../results/random_undersampling/class_distribution/%s-%s.%s" % (
        data.file_name, "naive_bayes", "png")

    # Split dataset and perform classification
    testable = data.as_testable()
    sampled_testable = common_preprocessing.random_under_sampling(testable)
...
for data in keel_data:
    # initialize file names and params
    csv_export_file_name = "../results/random_undersampling/data-random_forest.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/random_undersampling/confusion_matrix/%s-%s.%s" % (
...
for data in keel_data:
    # initialize file names and params
    csv_export_file_name = "../results/random_undersampling/data-decision_tree.csv"
    output_csv = open(csv_export_file_name, "a")
    confusion_matrix_plot_file_name = "../results/random_undersampling/confusion_matrix/%s-%s.%s" % (
